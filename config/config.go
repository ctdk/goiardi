...
type Conf struct {
	...
	StatsdTickInterval             int           `toml:"statsd-tick-interval"`
	StatsdResourcesCacheExpiration time.Duration `toml:"statsd-resources-cache-expiration"`
...
type Options struct {
	...
	StatsdTickInterval             int           `long:"statsd-tick-interval" description:"Statsd metrics collection interval in seconds (default: 10)" env:"GOIARDI_STATSD_TICK_INTERVAL"`
	StatsdResourcesCacheExpiration time.Duration `long:"statsd-resources-cache-expiration" description:"Expiration in minutes for chef resources metrics cache (default: 5m)" env:"GOIARDI_STATSD_RESOURCES_CACHE_EXPIRATION"`
...
func ParseConfigOptions() {
	...
	if opts.StatsdTickInterval != 0 {
		Config.StatsdTickInterval = opts.StatsdTickInterval
	}
	if opts.StatsdResourcesCacheExpiration != time.Duration(0)*time.Second {
		Config.StatsdResourcesCacheExpiration = opts.StatsdResourcesCacheExpiration
	}
	...
	if Config.StatsdTickInterval == 0 {
		Config.StatsdTickInterval = 10
	}
	if Config.StatsdResourcesCacheExpiration == time.Duration(0)*time.Second {
		Config.StatsdResourcesCacheExpiration = time.Duration(5) * time.Minute
	}
