...
func (p *PostgresSearch) Search(org *organization.Organization, idx string, q string, rows int, sortOrder string, start int, partialData map[string]interface{}) ([]map[string]interface{}, error) {
	...
	defer stmt.Close()
	var zzz int
	serr = stmt.QueryRow(org.GetId(), idx).Scan(&zzz) // don't care about zzz
	if serr != nil {
	...
	if q == "*:*" {
		...
		defer stmt.Close()
		if builtinIdx {
			err = stmt.QueryRow(org.GetId()).Scan(&res)
		} else {
			err = stmt.QueryRow(org.GetId(), idx).Scan(&res)
		}
		...
		pgQ := &PgQuery{idx: idx, queryChain: qchain, searchSchema: searchSchema, org: org}
...
func (pq *PgQuery) execute(startTableID ...*int) error {
	...
	for p != nil {
		...
		case *SubQuery:
			...
			p = nend
			np := &PgQuery{queryChain: newq, searchSchema: pq.searchSchema, org: pq.org}
			err := np.execute(t)
			if err != nil {
				return err
			}
	...
	fullQ, allArgs := craftFullQuery(pq.org.GetId(), pq.idx, pq.paths, pq.arguments, pq.queryStrs, t, pq.searchSchema)
...
func buildBasicQuery(field Field, term QueryTerm, tNum *int, op Op, searchSchema string) ([]string, string, string) {
	...
	if originalTerm == "*" || originalTerm == "" {
		...
		if notPath {
			q = fmt.Sprint("(" + q + " OR NOT EXISTS (SELECT 1 FROM found_items WHERE i.item_name = found_items.item_name AND found_items.path OPERATOR(goiardi.~) _ARG_))")
		}
		q = opStr + q
...
func craftFullQuery(orgID int64, idx string, paths []string, arguments []string, queryStrs []string, tNum *int, searchSchema string) (string, []interface{}) {
