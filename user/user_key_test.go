...
func TestGenerateNamedKeys(t *testing.T) {
	u, _ := New("foo_user_key_1")
	u.Save()

	priv, key, err := u.GenerateNamedKeys("default", infinity.Infinity)
	if err != nil {
...
func TestKeyZeroExpirationDate(t *testing.T) {
	u, _ := New("foo_user_key_2")
	u.Save()
	var tt time.Time
	priv, key, err := u.GenerateNamedKeys("default", tt)
	if err != nil {
...
func TestGetKey(t *testing.T) {
	u, _ := New("foo_user_key_4")
	u.Save()

	_, _, err := u.GenerateNamedKeys("default", infinity.Infinity)
	if err != nil {
		t.Errorf("generating default key failed: %s", err.Error())
	}
...
func TestUpdatingKey(t *testing.T) {
	...
	if k, ok := keys["default"]; !ok {
		...
		k.ExpirationDate = xt
		if xerr := u.SetNamedKey(k); xerr != nil {
			t.Errorf("Updating key error: %s", xerr.Error())
		}
		nk := u.NamedPublicKey("default")
...
func TestMultipleKey(t *testing.T) {
	u, _ := New("foo_user_key_6")
	u.Save()

	keyNames := []string{"default", "foober", "noomer"}

	for _, kName := range keyNames {
		_, _, err := u.GenerateNamedKeys(kName, infinity.Infinity)
		if err != nil {
