...
func policyACLHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	vars := mux.Vars(r)
	...
	it, iterr := policy.Get(org, vars["name"])
	if iterr != nil {
		jsonErrorReport(w, r, iterr.Error(), iterr.Status())
		return
	}
...
func policyACLPermHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	vars := mux.Vars(r)
